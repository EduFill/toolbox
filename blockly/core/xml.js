...
Blockly.Xml.blockToDom_ = function(block) {
  ...
  // marc.wollenweber@smail.inf.h-brs.de
  if (block.changeModeToDom) {
    // Custom data for an advanced block.
    var changemode = block.changeModeToDom();
    if (changemode) {
      element.appendChild(changemode);
    }
  }
  // end
...
Blockly.Xml.domToBlock_ = function(workspace, xmlBlock) {
  ...
  for (var x = 0, xmlChild; xmlChild = xmlBlock.childNodes[x]; x++) {
    ...
    switch (xmlChild.nodeName.toLowerCase()) {
      ...
      case 'changemode':
        if (block.domToChangeMode) {
          block.domToChangeMode(xmlChild);
        }
        break;
      // end
      default:
