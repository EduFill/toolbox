...
// The next functions are modfied by marc.wollenweber@smail.inf.h-brs.de (modification until next marker)
/**
 * Save xml blocks to local file.
 */
function xml_save() {
  var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
...
function py_save() {
	var data = Blockly.Generator.workspaceToCode('Python');
	// Store data in blob.
    var builder = new BlobBuilder();
    builder.append(data);
    saveAs(builder.getBlob('text/plain;charset=utf-8'), 'block.py');
}
// end of modification 
...
function auto_save_and_restore_blocks() {
  ...
  //modified by marc.wollenweber@smail.inf.h-brs.de 	
  // Init load event.
  /*
